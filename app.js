(function () {
  ...
  const vault = window.HISTORY_VAULT || [];
  ...
  function ensureSchedule(nowMs) {
    ...
    try {
      ...
      schedule = [];
      showMessage("HISTORY", "The documentary sources are being refreshed.", "Please try the channel again shortly.");
    }
  }
  ...
  function showStationCard(state) {
    ...
    if (state.segment.kind === "commercial") {
      ...
      showMessage("HISTORY", state.segment.title, `${formatDuration(state.segmentRemaining)} until the next documentary`);
  ...
  function tick() {
    ...
    els.mode.textContent = mode === "live" ? (state.segment.kind === "commercial" ? "LIVE · COMMERCIAL BREAK" : "LIVE HISTORY") : "TIME SHIFTED";
  ...
  async function shareChannel() {
    ...
    try {
      if (navigator.share) {
        ...
        els.shareStatus.textContent = "History Channel link copied.";
      }
    } catch (error) {
